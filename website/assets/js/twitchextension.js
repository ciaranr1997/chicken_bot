...
(()=>{
	...
	{
		$.ajax({
				 url: "/requests/extension/setbalance/"+Twitch.ext.viewer.id,
				 headers: {
	...
	async function getRedemptions()
	{
		...
		$.ajax({
				 url: "/requests/extension/redemptions"
		 }).then(function(data) {
				if(data)
				{
	...
	{
		$.ajax({
				 url: "/requests/extension/getbalance/"+Twitch.ext.viewer.id,
				 headers: {
	            'Content-Type':'application/json',
							'Authorization':Twitch.ext.viewer.sessionToken
	        }
	...
	function redeem(redemption,payload)
	{
		...
		$.ajax({
				 url: "/requests/extension/redeem/"+redemption,
				 type: 'POST',
				 data:payload,
				 headers: {
	            'Content-Type':'application/json',
	...
	function getRedemption(redeemId)
	{
		...
		$.ajax({
				 url: "/requests/extension/redemptionSettings/"+redeemId,
				 headers: {
    ...
    randomize:function()
    {
      ...
      else
      {
        ...
        rnd = Math.round(rnd);
			...
			if(rnd==this.currentval) {rnd = this.randomize}
      console.log(rnd);
      return rnd;
